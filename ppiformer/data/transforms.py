...
class PDBToPyGPretransform(T.BaseTransform):
    def __init__(
        ...
        # TODO Compression: Implement for 2x compression
...
# (e.g. 3BDY_HL_V DL27aN,PL27cA,RL27dK,SL28T,YL53F,TL93S,TL94S) in 'Mutation(s)_PDB'
class DDGLabelPretransform(T.BaseTransform):
    ...
    def __init__(
        self,
        df_path: Optional[Union[str, Path]] = None,
        ...
        # Load SKEMPI2 dataframe
        if df_path is not None:
            df_skempi = pd.read_csv(df_path)
        else:
...
class DockingScorePretransform(T.BaseTransform):
    ...
    def __call__(self, data: Data) -> Data:
        ...
        try:
            data.fnat = self.df[self.df['structure'] == name]['fnat'].values[0].astype('float32')
        except IndexError:
