...
class PPIformer(pl.LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        # self.save_hyperparameters(logger=False, ignore=['encoder', 'classifier'])  # NOTE: Also saves args passed to a child class
    ...
    # TODO Optimize stability by using raw logits as input to calc_log_odds (remove softmax)
    ...
    def predict_ddg(
        ...
        for p, s, w in zip(pos, sub, wt):
            if kind in ['masked_marginals', 'wt_marginals']:
                ...
                data.f = f
                ddg_pred.append(log_odds[p, s].sum())  # Simple additive model
    ...
    # NOTE: Nevertheless, it currently does not reproduce the results of the original implementation above. Should be debugged
    # def predict_ddg(
    #     self,
    #     data: Data,
    #     kind: DDG_INFERENCE_TYPE = 'wt_marginals'
